...
(define SOCKET-PATH (string-append (getenv "XDG_RUNTIME_DIR") "/hypr/" (getenv "HYPRLAND_INSTANCE_SIGNATURE") "/.socket2.sock"))
...
(define (call-hook hook params types)
  (apply run-hook
         (cons hook
               (fix-arguments-type
                (fix-arguments-length params (length types)) types))))
