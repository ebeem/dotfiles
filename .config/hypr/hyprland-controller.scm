...
(define* (hyprctl-get-grid-direction direction #:optional (current-workspace (hyprctl-get-active-workspace-id)))
  (let* ((current-workspace-group (floor (/ current-workspace WORKSPACES-FORMAT)))
         ...
         (target-row
          (case direction
            (("k" "up") (- current-row 1))
            (("j" "down") (+ current-row 1))
            (else current-row)))
         (target-column
          (case direction
            (("h" "left") (- current-column 1))
            (("l" "right") (+ current-column 1))
            (else current-column))))
...
(define cworkspace 14)
;; (display "\ncurrent-workspaces: ")
;; (display cworkspace)
;; (display "\ntarget-workspaces: ")
;; (display (hyprctl-get-grid-direction "k" cworkspace))
;; (hyprctl-move-to-workspace-direction "j")

(define (print-str . args)
  (display (string-join args)))

(print-str "current" "workspaces")
